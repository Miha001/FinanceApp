...
public class CurrenciesService(IMediator mediator) : ICurrenciesService
{
    ...
    public async Task<CollectionResult<CourseDto>> GetAll()
    {
        var result = await mediator.Send(new GetAllCurrenciesQuery());

        return CollectionResult<CourseDto>.Success(EntityToVM(result));
    }

    public async Task<CollectionResult<CourseDto>> GetCoursesByUserId(Guid userId)
    {
    ...
    private List<CourseDto> EntityToVM(IEnumerable<Currency> currencies)
    {
        return currencies.Select(x => new CourseDto(x.Id, x.Name, x.Rate)).ToList();
    }
}